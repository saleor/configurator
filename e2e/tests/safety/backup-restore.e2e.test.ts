...
describe("E2E Backup and Restore Tests", () => {
  ...
  describe("Automatic Backup Creation", () => {
    it("should create backup when overwriting existing configuration", async () => {
      ...
      const backupFiles = files.filter(f => f.includes("backup-overwrite-test") && f.includes(".bak"));
      ...
      expect(backupContent.shop.description).toBe("Original configuration that should be backed up");
    ...
    it("should create timestamped backup files", async () => {
      ...
      const files = await fs.readdir(testDir);
      const backupFiles = files.filter(f => f.includes("timestamped-backup-test") && f.includes(".bak"));
    ...
    it("should not create unnecessary backups when file doesn't exist", async () => {
      ...
      const files = await fs.readdir(testDir);
      const backupFiles = files.filter(f => f.includes("new-config-no-backup") && f.includes(".bak"));
  ...
  describe("Backup File Management", () => {
    it("should create readable and valid backup files", async () => {
      ...
      const files = await fs.readdir(testDir);
      const backupFiles = files.filter(f => f.includes("valid-backup-test") && f.includes(".bak"));
      ...
      const nestedCategory = backupContent.categories.find((c: any) => c.slug === "nested-category");
    ...
    it("should handle backup file naming conflicts", async () => {
      ...
      const files = await fs.readdir(testDir);
      const backupFiles = files.filter(f => f.includes("naming-conflict-test") && f.includes(".bak"));
  ...
  describe("Backup Content Integrity", () => {
    it("should preserve exact configuration state in backup", async () => {
      ...
      // Find backup file
      const files = await fs.readdir(testDir);
      const backupFiles = files.filter(f => f.includes("integrity-backup-test") && f.includes(".bak"));
      ...
      expect(backupContent.shop.defaultMailSenderName).toBe(complexConfig.shop.defaultMailSenderName);
      expect(backupContent.shop.defaultMailSenderAddress).toBe(complexConfig.shop.defaultMailSenderAddress);
      expect(backupContent.shop.description).toBe(complexConfig.shop.description);
      expect(backupContent.shop.trackInventoryByDefault).toBe(complexConfig.shop.trackInventoryByDefault);
      expect(backupContent.shop.defaultWeightUnit).toBe(complexConfig.shop.defaultWeightUnit);
      expect(backupContent.shop.automaticFulfillmentDigitalProducts).toBe(complexConfig.shop.automaticFulfillmentDigitalProducts);
      expect(backupContent.shop.fulfillmentAutoApprove).toBe(complexConfig.shop.fulfillmentAutoApprove);
      expect(backupContent.shop.defaultDigitalMaxDownloads).toBe(complexConfig.shop.defaultDigitalMaxDownloads);
      expect(backupContent.shop.defaultDigitalUrlValidDays).toBe(complexConfig.shop.defaultDigitalUrlValidDays);
      expect(backupContent.shop.reserveStockDurationAnonymousUser).toBe(complexConfig.shop.reserveStockDurationAnonymousUser);
      expect(backupContent.shop.reserveStockDurationAuthenticatedUser).toBe(complexConfig.shop.reserveStockDurationAuthenticatedUser);
      expect(backupContent.shop.limitQuantityPerCheckout).toBe(complexConfig.shop.limitQuantityPerCheckout);
      expect(backupContent.shop.enableAccountConfirmationByEmail).toBe(complexConfig.shop.enableAccountConfirmationByEmail);
      expect(backupContent.shop.allowLoginWithoutConfirmation).toBe(complexConfig.shop.allowLoginWithoutConfirmation);
    ...
    it("should handle special characters and formatting in backups", async () => {
      ...
      const files = await fs.readdir(testDir);
      const backupFiles = files.filter(f => f.includes("special-chars-backup-test") && f.includes(".bak"));
  ...
  describe("Backup Recovery Scenarios", () => {
    it("should enable recovery from deployment failures", async () => {
      ...
      let backupFiles = files.filter(f => f.includes("recovery-scenario-test") && f.includes(".bak"));
      expect(backupFiles.length).toBeGreaterThan(0);
      const backupPath = path.join(testDir, backupFiles[0]);
    ...
    it("should help with rollback after unwanted changes", async () => {
      ...
      // Find backup file
      const files = await fs.readdir(testDir);
      const backupFiles = files.filter(f => f.includes("rollback-scenario-test") && f.includes(".bak"));
      expect(backupFiles.length).toBeGreaterThan(0);
      const backupPath = path.join(testDir, backupFiles[0]);
  ...
  describe("Backup File Cleanup and Management", () => {
    ...
    it("should handle backup in different directory scenarios", async () => {
      ...
      const backupFiles = subDirFiles.filter(f => f.includes("nested-config") && f.includes(".bak"));
