...
describe("E2E Error Recovery and Partial Failure Tests", () => {
  ...
  beforeAll(async () => {
    ...
    container = new SaleorTestContainer({
      projectName: "saleor-error-recovery-test",
    });
  ...
  describe("Partial Failure Recovery", () => {
    ...
    it("should maintain data integrity after deployment errors", async () => {
      ...
      const baselineChannel = integrityState.channels?.find((c: any) => c.slug === "baseline-channel");
      ...
      const baselineCategory = integrityState.categories?.find((c: any) => c.slug === "baseline-category");
  ...
  describe("Network and Connection Recovery", () => {
    ...
    it("should recover from connection interruptions", async () => {
      ...
      const connectionErrorResult = await cli.deploy("http://invalid-host-that-does-not-exist.com/graphql/", token, {
        config: configPath,
        timeout: 10000,
        skipDiff: true
      });
