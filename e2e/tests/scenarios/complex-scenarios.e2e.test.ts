...
describe("E2E Complex Scenarios", () => {
  ...
  beforeAll(async () => {
    ...
    container = new SaleorTestContainer({
      projectName: "saleor-complex-test",
    });
  ...
  describe("Multi-Channel E-commerce Setup", () => {
    it("should handle complex multi-region setup with products and channels", async () => {
            ...
            productAttributes: [
              {
                ...
                type: "DROPDOWN",
                inputType: "DROPDOWN",
                choices: ["Apple", "Samsung", "Sony", "Microsoft", "Nintendo", "Dell", "HP", "Lenovo"]
              },
              {
          ...
          },
          {
            ...
            variantAttributes: [
              ...
              },
              {
                ...
                type: "DROPDOWN",
                inputType: "DROPDOWN",
                choices: ["Black", "White", "Navy", "Gray", "Red", "Blue", "Green", "Yellow", "Pink"]
              }
            ]
          }
        ],
      ...
      const electronicsType = verifyConfig.productTypes.find((pt: any) => pt.slug === "electronics");
      ...
      const brandAttr = electronicsType.productAttributes.find((attr: any) => attr.slug === "brand");
      const materialAttr = clothingType.productAttributes.find((attr: any) => attr.slug === "material");
      ...
      const landingPageType = verifyConfig.pageTypes.find((pt: any) => pt.slug === "product-landing-page");
  ...
  describe("Incremental Configuration Changes", () => {
    it("should handle progressive configuration expansion", async () => {
      ...
      expect(finalConfig.productTypes.find((pt: any) => pt.slug === "furniture-type")).toBeDefined();
  ...
  describe("Configuration Rollback and Recovery", () => {
    it("should handle configuration rollback scenarios", async () => {
      ...
      expect(modifiedVerifyConfig.channels.find((c: any) => c.slug === "modified-channel")).toBeDefined();
      expect(modifiedVerifyConfig.categories.find((c: any) => c.slug === "modified-category")).toBeDefined();
      expect(modifiedVerifyConfig.productTypes?.find((pt: any) => pt.slug === "new-product-type")).toBeDefined();
      ...
      expect(rollbackVerifyConfig.channels.find((c: any) => c.slug === "rollback-channel")).toBeDefined();
      expect(rollbackVerifyConfig.categories.find((c: any) => c.slug === "original-category")).toBeDefined();
      
      // Verify modified entities are no longer present
      expect(rollbackVerifyConfig.channels.find((c: any) => c.slug === "modified-channel")).toBeUndefined();
      expect(rollbackVerifyConfig.categories.find((c: any) => c.slug === "modified-category")).toBeUndefined();
