...
describe("DiffService", () => {
  ...
  // Helper function to safely access configStorage
  const getConfigStorage = () => {
    if (!mockServices.configStorage) throw new Error("configStorage not mocked");
    return mockServices.configStorage;
  };

  // Helper function to safely access configuration
  const getConfiguration = () => {
    if (!mockServices.configuration) throw new Error("configuration not mocked");
    return mockServices.configuration;
  };
  ...
  beforeEach(() => {
    ...
    } as unknown as Partial<ServiceContainer>;

    diffService = new DiffService(mockServices as import("../service-container").ServiceContainer);
  });
  ...
  describe("compareChannels", () => {
    it("should detect channel creation", async () => {
      ...
      getConfigStorage().load = vi.fn().mockResolvedValue(localConfig);
      getConfiguration().retrieveWithoutSaving = vi.fn().mockResolvedValue(remoteConfig);
  ...
  describe("error handling", () => {
    it("should handle configuration loading errors", async () => {
      ...
      getConfigStorage().load = vi.fn().mockRejectedValue(new Error("Config file not found"));
    ...
    it("should handle remote configuration retrieval errors", async () => {
      ...
      getConfigStorage().load = vi.fn().mockResolvedValue({});
      getConfiguration().retrieveWithoutSaving = vi
        .fn()
    ...
    it("should handle timeout for remote configuration", async () => {
      ...
      const serviceWithTimeout = new DiffService(
      ...
      getConfigStorage().load = vi.fn().mockResolvedValue({});
      getConfiguration().retrieveWithoutSaving = vi.fn().mockImplementation(
  ...
  describe("no differences", () => {
    it("should return empty summary when configs are identical", async () => {
      ...
      getConfigStorage().load = vi.fn().mockResolvedValue(config);
      getConfiguration().retrieveWithoutSaving = vi.fn().mockResolvedValue(config);
  ...
  describe("service configuration", () => {
    it("should use custom configuration", async () => {
      ...
      const serviceWithDebug = new DiffService(
        mockServices as import("../service-container").ServiceContainer,
        {
  ...
  describe("performance", () => {
    it("should handle large configurations efficiently", async () => {
      ...
      getConfigStorage().load = vi.fn().mockResolvedValue(largeConfig);
      getConfiguration().retrieveWithoutSaving = vi.fn().mockResolvedValue(emptyConfig);
  ...
  describe("edge cases", () => {
    it("should handle empty configurations", async () => {
      ...
      getConfigStorage().load = vi.fn().mockResolvedValue(emptyConfig);
      getConfiguration().retrieveWithoutSaving = vi.fn().mockResolvedValue(emptyConfig);
