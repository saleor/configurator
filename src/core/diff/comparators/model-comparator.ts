...
> {
  ...
  private normalizeAttributeValue(value: any): any {
  ...
  private getAttributes(entity: ModelInput | Page): Record<string, any> {
    if ("attributes" in entity && entity.attributes) {
      // For local input, attributes are already a key-value record
      return entity.attributes as Record<string, any>;
    }
        ...
        if (slug) {
          ...
          if (attr.values && attr.values.length > 0) {
            ...
            attributes[slug] = attr.values
              .map((v: any) => v.name || v.slug || v.value || v)
              .filter(Boolean);
          } else if (attr.value) {
            // Single value attribute
            attributes[slug] = attr.value;
          }
        }
      }
    }
  ...
  protected compareEntityFields(local: ModelInput, remote: Page): DiffChange[] {
    ...
    const localType = local.modelType;
