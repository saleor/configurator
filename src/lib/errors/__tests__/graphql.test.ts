...
describe("GraphQLError", () => {
  ...
  describe("fromGraphQLErrors", () => {
    ...
    it("should handle multiple permissions", () => {
      ...
      expect(error.message).toContain("You need one of the following permissions: MANAGE_PRODUCTS, MANAGE_STAFF");
    });
    ...
    it("should extract field from error message if not in extensions", () => {
      ...
      expect(error.message).toContain("Variable \"$name\" of required type \"String!\" was not provided.");
    });
  });
  ...
  describe("fromCombinedError", () => {
    ...
    it("should detect 403 forbidden errors", () => {
      ...
      expect(error.message).toContain("Your authentication token doesn't have the required permissions");
    });
