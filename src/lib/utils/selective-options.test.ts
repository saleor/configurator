...
describe('selective-options utility', () => {
  describe('parseSelectiveOptions', () => {
    ...
    it('should throw error for invalid only sections', () => {
      ...
      expect(() => parseSelectiveOptions(options)).toThrow(
        `Invalid sections specified in --only: invalid,unknown. Available: ${AVAILABLE_SECTIONS.join(', ')}`
      );
    });
    ...
    it('should throw error for invalid exclude sections', () => {
      ...
      // Act & Assert
      expect(() => parseSelectiveOptions(options)).toThrow(
        `Invalid sections specified in --exclude: invalid,unknown. Available: ${AVAILABLE_SECTIONS.join(', ')}`
      );
    });
  });
  ...
  describe('shouldIncludeSection', () => {
    it('should include section when only specific sections are included', () => {
      // Arrange
      const options = { includeSections: ['channels', 'shop'] as ConfigurationSection[], excludeSections: [] as ConfigurationSection[] };
    ...
    it('should exclude section when sections are excluded', () => {
      // Arrange
      const options = { includeSections: [] as ConfigurationSection[], excludeSections: ['products', 'categories'] as ConfigurationSection[] };
    ...
    it('should include all sections when no selective options', () => {
      // Arrange
      const options = { includeSections: [] as ConfigurationSection[], excludeSections: [] as ConfigurationSection[] };
